...
struct State {
    graph: gpu::Graph,
    window: Window,
    compute_pipeline: gpu::Pipeline,
    graphics_pipeline: gpu::Pipeline,
    storage_image: gpu::Image,
    sampler: gpu::Sampler,
}
...
impl common::Example for State {
    ...
    fn draw(&mut self) -> Result<(), gpu::GraphError> {
        ...
        frame
            .compute_pass("fill")
            .write((self.storage_image, gpu::Access::ComputeWrite))
            .execute(|cmd| {
        ...
        frame
            .render_pass("blit")
            .read((self.storage_image, gpu::Access::ShaderRead))
